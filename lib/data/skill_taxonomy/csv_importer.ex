defmodule Data.SkillTaxonomy.CsvImporter do
  @moduledoc """
  Parses the skill-taxonomy CSV format (see `design/SKILLS_TAXONOMY.md`
  §4) into `Role`/`Skill` document maps and *pending* relations
  (addressed by natural key, not yet a real `@id`). Pure — no network.
  Per-row document building delegates to `Data.SkillTaxonomy.RowBuilder`,
  shared with the LiveView entry form and `Data.SkillTaxonomy.XlsxImporter`;
  this module only handles what's CSV-specific (splitting `;`-delimited
  cells, and the cross-row checks `RowBuilder` can't do on its own:
  duplicate role identity, whether a `context` row's base role exists
  elsewhere in the same file).

  The actual TerminusDB writes — inserting roles/skills, then resolving
  and inserting `RoleRelation` documents from the ids those inserts
  return — live in `Data.SkillTaxonomy.Importer.import/2`, which takes
  this module's `parse/1` output (or `XlsxImporter`'s) as-is; see that
  module's moduledoc for why `RoleRelation.from`/`to` can't be resolved
  here.
  """
